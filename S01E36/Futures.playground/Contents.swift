...
extension Episode {
    static let all = Resource<[Episode]>(url: URL(string: "http://localhost:8080/episodes.json")!, parseJSON: { json in
        guard let dictionaries = json as? [JSONDictionary] else { return nil }
        return dictionaries.flatMap(Episode.init)
    })
    
    var details: Resource<EpisodeDetails> {
        let url = URL(string: "http://localhost:8080/episodes/\(id).json")!
...
final class Future<A> {
    ...
    func onResult(_ callback: @escaping (Result<A>) -> ()) -> Future<A> {
